...
function renderBody(body) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!line) continue;
    if (line.startsWith("## ")) {
      elements.push(<h2 key={i} style={{ fontFamily: SF, fontSize: 26, fontWeight: 300, color: INK, marginTop: 40, marginBottom: 12 }}>{line.replace("## ", "")}</h2>);
    } else if (line.startsWith("**") && line.includes("**")) {
      const parts = line.split("**");
      elements.push(<p key={i} style={{ fontSize: 15, color: "#444", lineHeight: 1.75, marginBottom: 12 }}>{parts.map((part, j) => j % 2 === 1 ? <strong key={j} style={{ color: INK }}>{part}</strong> : <span key={j}>{part}</span>)}</p>);
    } else {
      elements.push(<p key={i} style={{ fontSize: 15, color: "#444", lineHeight: 1.75, marginBottom: 12 }}>{line}</p>);
    }
  }
  return elements;
}
      ...
      <article style={{ maxWidth: 740, margin: "0 auto", padding: "48px 32px" }}>
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: "flex", gap: 12, marginBottom: 16, alignItems: "center" }}>
            {post.category && <><span style={{ fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: "#999" }}>{post.category}</span><span style={{ fontSize: 11, color: "#ccc" }}>&middot;</span></>}
        ...
        {/* BODY FORMAT: template literal string */}
        {post.body && (
          <div>{renderBody(post.body)}</div>
        )}

        {/* RANKINGS FORMAT: structured hoods array */}
        {post.hoods && (
          <>
            <div style={{ fontFamily: SF, fontSize: 18, color: "#444", lineHeight: 1.7, marginBottom: 32 }}>
              <p>We analysed all 158 Toronto neighbourhoods using the latest available data from Toronto Police Service, Statistics Canada, the Fraser Institute, and City of Toronto Open Data. Every ranking below is driven by real numbers.</p>
            </div>
            <div>
              {post.hoods.map((h, i) => (
                <div key={h.slug || i} style={{ marginBottom: 32, paddingBottom: 32, borderBottom: i < post.hoods.length - 1 ? "1px solid #eee" : "none" }}>
        ...
        <div style={{ fontFamily: SF, fontSize: 18, color: "#444", lineHeight: 1.7, marginTop: 32, marginBottom: 32, paddingTop: 32, borderTop: "1px solid #eee" }}>
          ...
          <p>Rankings are based on data from Toronto Open Data, Statistics Canada Census 2021, Toronto Police Service, Fraser Institute, OCHPP, and City of Toronto. All figures reflect the most recent available data as of March 2026.</p>
        </div>
